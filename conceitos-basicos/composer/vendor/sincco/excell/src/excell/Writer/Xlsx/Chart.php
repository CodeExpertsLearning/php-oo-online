...
use Sincco\Excell\Chart\Axis;
use Sincco\Excell\Chart\DataSeries;
use Sincco\Excell\Chart\DataSeriesValues;
use Sincco\Excell\Chart\GridLines;
use Sincco\Excell\Chart\Layout;
use Sincco\Excell\Chart\Legend;
use Sincco\Excell\Chart\PlotArea;
use Sincco\Excell\Chart\Title;
...
class Chart extends WriterPart
{
     ...
     *
     * @param  \Sincco\Excell\Chart $pChart
     *
     * @throws  \Sincco\Excell\Writer\Exception
     * @return  string            XML Output
     */
    public function writeChart(\Sincco\Excell\Chart $pChart = null, $calculateCellValues = true)
    {
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writeTitle(\Sincco\Excell\Shared\XMLWriter $objWriter, Title $title = null)
    {
        if (is_null($title)) {
            return;
        }
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writeLegend(\Sincco\Excell\Shared\XMLWriter $objWriter, Legend $legend = null)
    {
        if (is_null($legend)) {
            return;
        }
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writePlotArea(\Sincco\Excell\Shared\XMLWriter $objWriter, \Sincco\Excell\Worksheet $pSheet, PlotArea $plotArea, Title $xAxisLabel = null, Title $yAxisLabel = null, Axis $xAxis = null, Axis $yAxis = null, GridLines $majorGridlines = null, GridLines $minorGridlines = null)
    {
        if (is_null($plotArea)) {
            return;
        }
     ...
     *
     * @param  \Sincco\Excell\Shared\XMLWriter $objWriter XML Writer
     * @param  \Sincco\Excell\Chart\Layout $chartLayout Chart layout
     *
     * @throws  \Sincco\Excell\Writer\Exception
     */
    ...
    private function writeCategoryAxis($objWriter, PlotArea $plotArea, $xAxisLabel, $groupType, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis)
    {
        ...
        if (!is_null($xAxisLabel)) {
            ...
            $objWriter->writeRawData(\Sincco\Excell\Shared\StringHelper::controlCharacterPHP2OOXML($caption));
            $objWriter->endElement();
    ...
    private static function getChartType($plotArea)
    {
        ...
        if ($groupCount == 1) {
            ...
            if (count($chartTypes) == 0) {
                throw new \Sincco\Excell\Writer\Exception('Chart is not yet implemented');
            }
        }
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writePlotGroup($plotGroup, $groupType, $objWriter, &$catIsMultiLevelSeries, &$valIsMultiLevelSeries, &$plotGroupingType, \Sincco\Excell\Worksheet $pSheet)
    {
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writePlotSeriesValues($plotSeriesValues, \Sincco\Excell\Shared\XMLWriter $objWriter, $groupType, $dataType = 'str')
    {
     ...
     * @throws  \Sincco\Excell\Writer\Exception
     */
    private function writeLayout(\Sincco\Excell\Shared\XMLWriter $objWriter, Layout $layout = null)
    {
