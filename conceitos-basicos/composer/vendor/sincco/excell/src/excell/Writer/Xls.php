...
class Xls extends BaseWriter implements IWriter
{
    ...
    public function save($pFilename = null)
    {
        ...
        $OLE = new \Sincco\Excell\Shared\OLE\PPS\File(\Sincco\Excell\Shared\OLE::ascToUcs($workbookStreamName));
        ...
        if (isset($this->documentSummaryInformation) && !empty($this->documentSummaryInformation)) {
            $OLE_DocumentSummaryInformation = new \Sincco\Excell\Shared\OLE\PPS\File(\Sincco\Excell\Shared\OLE::ascToUcs(chr(5) . 'DocumentSummaryInformation'));
        ...
        if (isset($this->summaryInformation) && !empty($this->summaryInformation)) {
            $OLE_SummaryInformation = new \Sincco\Excell\Shared\OLE\PPS\File(\Sincco\Excell\Shared\OLE::ascToUcs(chr(5) . 'SummaryInformation'));
        ...
        $root = new \Sincco\Excell\Shared\OLE\PPS\Root(time(), time(), $arrRootData);
        // save the OLE file
     ...
     * @return \Sincco\Excell\Writer\Xls
     */
    ...
    private function buildWorksheetEschers()
    {
        ...
        foreach ($this->spreadsheet->getAllsheets() as $sheet) {
            ...
            $escher = new \Sincco\Excell\Shared\Escher();

            // dgContainer
            $dgContainer = new \Sincco\Excell\Shared\Escher\DgContainer();
            ...
            // spgrContainer
            $spgrContainer = new \Sincco\Excell\Shared\Escher\DgContainer\SpgrContainer();
            $dgContainer->setSpgrContainer($spgrContainer);

            // add one shape which is the group shape
            $spContainer = new \Sincco\Excell\Shared\Escher\DgContainer\SpgrContainer\SpContainer();
            ...
            foreach ($sheet->getDrawingCollection() as $drawing) {
                ...
                $twoAnchor = \Sincco\Excell\Shared\Xls::oneAnchor2twoAnchor($sheet, $coordinates, $offsetX, $offsetY, $width, $height);
            ...
            if (!empty($filterRange)) {
                $rangeBounds = \Sincco\Excell\Cell::rangeBoundaries($filterRange);
                ...
                while ($iInc <= $iNumColEnd) {
                    ...
                    $oDrawing = new \Sincco\Excell\Worksheet\BaseDrawing();
                    // get the coordinates of drawing
                    $cDrawing = \Sincco\Excell\Cell::stringFromColumnIndex($iInc - 1) . $rangeBounds[0][1];
                    ...
                    // set coordinates and offsets, client anchor
                    $endCoordinates = \Sincco\Excell\Cell::stringFromColumnIndex($iInc - 1);
    ...
    private function buildWorkbookEscher()
    {
        ...
        // dggContainer
        $dggContainer = new \Sincco\Excell\Shared\Escher\DggContainer();
        ...
        // bstoreContainer
        $bstoreContainer = new \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer();
                ...
                if ($drawing instanceof \Sincco\Excell\Worksheet\Drawing) {
                    ...
                    switch ($imageFormat) {
                        case 1: // GIF, not supported by BIFF8, we convert to PNG
                            $blipType = \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_PNG;
                            ob_start();
                            ...
                            break;
                        case 2: // JPEG
                            $blipType = \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_JPEG;
                        ...
                        case 6: // Windows DIB (BMP), we convert to PNG
                            $blipType = \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_PNG;
                            ob_start();
                            imagepng(\Sincco\Excell\Shared\Drawing::imagecreatefrombmp($filename));
                            $blipData = ob_get_contents();
                            ob_end_clean();
                            break;
                        default:
                    ...
                    $blip = new \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE\Blip();
                    $blip->setData($blipData);

                    $BSE = new \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE();
                    ...
                    switch ($drawing->getRenderingFunction()) {
                        case \Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_JPEG:
                            $blipType = \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_JPEG;
                            $renderingFunction = 'imagejpeg';
                            break;
                        case \Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_GIF:
                        case \Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_PNG:
                        case \Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_DEFAULT:
                            $blipType = \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_PNG;
    ...
    private function writeSummaryInformation()
    {
        ...
        if ($this->spreadsheet->getProperties()->getCreated()) {
            ...
            $dataSection[] = [
                ...
                'data' => ['data' => \Sincco\Excell\Shared\OLE::localDateToOLE($dataProp)],
            ];
            ++$dataSection_NumProps;
        }
        //    Modified Date/Time
