...
class Style extends WriterPart
{
     ...
     */
    public function writeStyles(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
        // Create XML writer
        $objWriter = null;
     ...
     * @param     \Sincco\Excell\Style\Fill            $pFill            Fill style
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeFill(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\Fill $pFill = null)
    {
        // Check if this is a pattern type or gradient type
        if ($pFill->getFillType() === \Sincco\Excell\Style\Fill::FILL_GRADIENT_LINEAR ||
            $pFill->getFillType() === \Sincco\Excell\Style\Fill::FILL_GRADIENT_PATH) {
            // Gradient fill
     ...
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeGradientFill(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\Fill $pFill = null)
    {
        // fill
     ...
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writePatternFill(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\Fill $pFill = null)
    {
        // fill
        $objWriter->startElement('fill');
        ...
        if ($pFill->getFillType() !== \Sincco\Excell\Style\Fill::FILL_NONE) {
            // fgColor
     ...
     *
     * @param     \Sincco\Excell\Shared\XMLWriter        $objWriter         XML Writer
     * @param     \Sincco\Excell\Style\Font                $pFont            Font style
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeFont(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\Font $pFont = null)
    {
        // font
     ...
     *
     * @param     \Sincco\Excell\Shared\XMLWriter            $objWriter         XML Writer
     * @param     \Sincco\Excell\Style\Borders                $pBorders        Borders style
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeBorder(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\Borders $pBorders = null)
    {
        // Write border
        $objWriter->startElement('border');
        // Diagonal?
        switch ($pBorders->getDiagonalDirection()) {
            case \Sincco\Excell\Style\Borders::DIAGONAL_UP:
                $objWriter->writeAttribute('diagonalUp', 'true');
                $objWriter->writeAttribute('diagonalDown', 'false');
                break;
            case \Sincco\Excell\Style\Borders::DIAGONAL_DOWN:
                $objWriter->writeAttribute('diagonalUp', 'false');
                $objWriter->writeAttribute('diagonalDown', 'true');
                break;
            case \Sincco\Excell\Style\Borders::DIAGONAL_BOTH:
                $objWriter->writeAttribute('diagonalUp', 'true');
                $objWriter->writeAttribute('diagonalDown', 'true');
                break;
        }
     ...
     *
     * @param     \Sincco\Excell\Shared\XMLWriter            $objWriter         XML Writer
     * @param     \Sincco\Excell\Style                        $pStyle            Style
     * @param \Sincco\Excell\SpreadSheet     $spreadsheet        Workbook
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeCellStyleXf(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style $pStyle = null, \Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
        // xf
        $objWriter->startElement('xf');
        $objWriter->writeAttribute('xfId', 0);
        ...
        if ($pStyle->getProtection()->getLocked() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT || $pStyle->getProtection()->getHidden() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT) {
        ...
        if ($pStyle->getProtection()->getLocked() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT || $pStyle->getProtection()->getHidden() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT) {
            ...
            if ($pStyle->getProtection()->getLocked() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT) {
                $objWriter->writeAttribute('locked', ($pStyle->getProtection()->getLocked() == \Sincco\Excell\Style\Protection::PROTECTION_PROTECTED ? 'true' : 'false'));
            }
            if ($pStyle->getProtection()->getHidden() != \Sincco\Excell\Style\Protection::PROTECTION_INHERIT) {
                $objWriter->writeAttribute('hidden', ($pStyle->getProtection()->getHidden() == \Sincco\Excell\Style\Protection::PROTECTION_PROTECTED ? 'true' : 'false'));
            }
            $objWriter->endElement();
        }
     ...
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeCellStyleDxf(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style $pStyle = null)
    {
        // dxf
        ...
        if (($pStyle->getProtection()->getLocked() !== null) || ($pStyle->getProtection()->getHidden() !== null)) {
            if ($pStyle->getProtection()->getLocked() !== \Sincco\Excell\Style\Protection::PROTECTION_INHERIT ||
                $pStyle->getProtection()->getHidden() !== \Sincco\Excell\Style\Protection::PROTECTION_INHERIT) {
                $objWriter->startElement('protection');
                if (($pStyle->getProtection()->getLocked() !== null) &&
                    ($pStyle->getProtection()->getLocked() !== \Sincco\Excell\Style\Protection::PROTECTION_INHERIT)) {
                    $objWriter->writeAttribute('locked', ($pStyle->getProtection()->getLocked() == \Sincco\Excell\Style\Protection::PROTECTION_PROTECTED ? 'true' : 'false'));
                }
     ...
     * @param     \Sincco\Excell\Style\Border            $pBorder        Border style
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeBorderPr(\Sincco\Excell\Shared\XMLWriter $objWriter = null, $pName = 'left', \Sincco\Excell\Style\Border $pBorder = null)
    {
        // Write BorderPr
        if ($pBorder->getBorderStyle() != \Sincco\Excell\Style\Border::BORDER_NONE) {
     ...
     *
     * @param     \Sincco\Excell\Shared\XMLWriter            $objWriter         XML Writer
     * @param     \Sincco\Excell\Style\NumberFormat            $pNumberFormat    Number Format
     * @param     int                                    $pId            Number Format identifier
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeNumFmt(\Sincco\Excell\Shared\XMLWriter $objWriter = null, \Sincco\Excell\Style\NumberFormat $pNumberFormat = null, $pId = 0)
    {
     ...
     * @param     \Sincco\Excell\Spreadsheet                $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style[]        All styles in Excell
     */
    public function allStyles(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
     ...
     * @param \Sincco\Excell\SpreadSheet     $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style\Conditional[]        All conditional styles in Excell
     */
    public function allConditionalStyles(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
        // Get an array of all styles
        $aStyles = [];
     ...
     * @param \Sincco\Excell\SpreadSheet $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style\Fill[]        All fills in Excell
     */
    public function allFills(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
        // Get an array of unique fills
        $aFills = [];

        // Two first fills are predefined
        $fill0 = new \Sincco\Excell\Style\Fill();
        $fill0->setFillType(\Sincco\Excell\Style\Fill::FILL_NONE);
        $aFills[] = $fill0;

        $fill1 = new \Sincco\Excell\Style\Fill();
        $fill1->setFillType(\Sincco\Excell\Style\Fill::FILL_PATTERN_GRAY125);
        $aFills[] = $fill1;
        // The remaining fills
        $aStyles = $this->allStyles($spreadsheet);
        /** @var \Sincco\Excell\Style $style */
     ...
     * @param \Sincco\Excell\SpreadSheet $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style\Font[]        All fonts in Excell
     */
    public function allFonts(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
     ...
     * @param \Sincco\Excell\SpreadSheet $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style\Borders[]        All borders in Excell
     */
    public function allBorders(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
     ...
     * @param \Sincco\Excell\SpreadSheet $spreadsheet
     * @throws     \Sincco\Excell\Writer\Exception
     * @return     \Sincco\Excell\Style\NumberFormat[]        All number formats in Excell
     */
    public function allNumberFormats(\Sincco\Excell\SpreadSheet $spreadsheet = null)
    {
