...
class Workbook extends BIFFwriter
{
    ...
    /**
     * XF Writers
     * @var \Sincco\Excell\Writer\Xls\Xf[]
     */
    private $xfWriters = [];
     ...
     * Fonts writers
     *
     * @var \Sincco\Excell\Writer\Xls\Font[]
     */
    private $fontWriters = [];
     ...
     * Class constructor
     *
     * @param \Sincco\Excell\Spreadsheet $spreadsheet The Workbook
     ...
     */
    public function __construct(\Sincco\Excell\Spreadsheet $spreadsheet, &$str_total, &$str_unique, &$str_table, &$colors, Parser $parser)
    {
     ...
     *
     * @param \Sincco\Excell\Style\Font $font
     * @return int Index to FONT record
     */
    public function addFont(\Sincco\Excell\Style\Font $font)
    {
        ...
        if (isset($this->addedFonts[$fontHashCode])) {
            ...
            $fontWriter = new \Sincco\Excell\Writer\Xls\Font($font);
    ...
    private function calcSheetOffsets()
    {
        ...
        foreach ($this->spreadsheet->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + strlen(\Sincco\Excell\Shared\StringHelper::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
    ...
    private function writeNames()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                // Print area
                $printArea = \Sincco\Excell\Cell::splitRange($sheetSetup->getPrintArea());
                $printArea = $printArea[0];
                $printArea[0] = \Sincco\Excell\Cell::coordinateFromString($printArea[0]);
                $printArea[1] = \Sincco\Excell\Cell::coordinateFromString($printArea[1]);

                $print_rowmin = $printArea[0][1] - 1;
                $print_rowmax = $printArea[1][1] - 1;
                $print_colmin = \Sincco\Excell\Cell::columnIndexFromString($printArea[0][0]) - 1;
                $print_colmax = \Sincco\Excell\Cell::columnIndexFromString($printArea[1][0]) - 1;
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isColumnsToRepeatAtLeftSet() && $sheetSetup->isRowsToRepeatAtTopSet()) {
                ...
                $colmin = \Sincco\Excell\Cell::columnIndexFromString($repeat[0]) - 1;
                $colmax = \Sincco\Excell\Cell::columnIndexFromString($repeat[1]) - 1;
    ...
    private function writeAllDefinedNamesBiff8()
    {
        ...
        if (count($this->spreadsheet->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                $range = \Sincco\Excell\Cell::splitRange($namedRange->getRange());
                for ($i = 0; $i < count($range); ++$i) {
                    $range[$i][0] = '\'' . str_replace("'", "''", $namedRange->getWorksheet()->getTitle()) . '\'!' . \Sincco\Excell\Cell::absoluteCoordinate($range[$i][0]);
                    if (isset($range[$i][1])) {
                        $range[$i][1] = \Sincco\Excell\Cell::absoluteCoordinate($range[$i][1]);
                    }
                }
                $range = \Sincco\Excell\Cell::buildRange($range); // e.g. Sheet1!$A$1:$B$2
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                for ($j = 0; $j < $countPrintArea; ++$j) {
                    ...
                    $printAreaRect[0] = \Sincco\Excell\Cell::coordinateFromString($printAreaRect[0]);
                    $printAreaRect[1] = \Sincco\Excell\Cell::coordinateFromString($printAreaRect[1]);

                    $print_rowmin = $printAreaRect[0][1] - 1;
                    $print_rowmax = $printAreaRect[1][1] - 1;
                    $print_colmin = \Sincco\Excell\Cell::columnIndexFromString($printAreaRect[0][0]) - 1;
                    $print_colmax = \Sincco\Excell\Cell::columnIndexFromString($printAreaRect[1][0]) - 1;
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if (!empty($autoFilterRange)) {
                $rangeBounds = \Sincco\Excell\Cell::rangeBoundaries($autoFilterRange);
    ...
    private function writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
    {
        ...
        $nlen = \Sincco\Excell\Shared\StringHelper::countCharacters($name);

        // name with stripped length field
        $name = substr(\Sincco\Excell\Shared\StringHelper::UTF8toBIFF8UnicodeLong($name), 2);
    ...
    private function writeWindow1()
    {
        ...
        // not supported by Excell, so there is only one selected sheet, the active
        $ctabsel = 1; // Number of workbook tabs selected

        $wTabRatio = 0x0258; // Tab to scrollbar ratio

        // not supported by Excell, set to 0
     ...
     *
     * @param \Sincco\Excell\Worksheet  $sheet Worksheet name
    ...
    private function writeBoundSheet($sheet, $offset)
    {
        ...
        switch ($sheet->getSheetState()) {
            case \Sincco\Excell\Worksheet::SHEETSTATE_VISIBLE:
                $ss = 0x00;
                break;
            case \Sincco\Excell\Worksheet::SHEETSTATE_HIDDEN:
                $ss = 0x01;
                break;
            case \Sincco\Excell\Worksheet::SHEETSTATE_VERYHIDDEN:
                $ss = 0x02;
                break;
            default:
                $ss = 0x00;
        ...
        $data .= \Sincco\Excell\Shared\StringHelper::UTF8toBIFF8UnicodeShort($sheetname);
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $numberFormatString = \Sincco\Excell\Shared\StringHelper::UTF8toBIFF8UnicodeLong($format);
    ...
    private function writeDateMode()
    {
        ...
        $f1904 = (\Sincco\Excell\Shared\Date::getExcelCalendar() == \Sincco\Excell\Shared\Date::CALENDAR_MAC_1904)
            ? 1
