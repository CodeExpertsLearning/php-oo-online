...
class ContentTypes extends WriterPart
{
     ...
     */
    public function writeContentTypes(\Sincco\Excell\Spreadsheet $spreadsheet = null, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        ...
        for ($i = 0; $i < $mediaCount; ++$i) {
            ...
            if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof \Sincco\Excell\Worksheet\Drawing) {
                $extension = strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
                $mimeType = $this->getImageMimeType($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getPath());
            } elseif ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof \Sincco\Excell\Worksheet\MemoryDrawing) {
    ...
    {
        if (\Sincco\Excell\Shared\File::fileExists($pFile)) {
            $image = getimagesize($pFile);

            return image_type_to_mime_type($image[2]);
        } else {
            throw new \Sincco\Excell\Writer\Exception("File $pFile does not exist");
        }
    }
     ...
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeDefaultContentType(\Sincco\Excell\Shared\XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
    {
     ...
     * @throws     \Sincco\Excell\Writer\Exception
     */
    private function writeOverrideContentType(\Sincco\Excell\Shared\XMLWriter $objWriter = null, $pPartname = '', $pContentType = '')
    {
        if ($pPartname != '' && $pContentType != '') {
            // Write content type
