...
class PDF implements IWriter
{
    ...
    {
        $pdfLibraryName = \Sincco\Excell\Settings::getPdfRendererName();
        if (is_null($pdfLibraryName)) {
            throw new Exception('PDF Rendering library has not been defined.');
        }

        $pdfLibraryPath = \Sincco\Excell\Settings::getPdfRendererPath();
        if (is_null($pdfLibraryName)) {
        ...
        $rendererName = '\\Sincco\\Excell\\Writer\\PDF\\' . $pdfLibraryName;
