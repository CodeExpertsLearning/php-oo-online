...
class AutoFilter
{
     ...
     */
    public function __construct($pRange = '', \Sincco\Excell\Worksheet $pSheet = null)
    {
        $this->range = $pRange;
     ...
     *
     * @return \Sincco\Excell\Worksheet
     */
     ...
     * @return AutoFilter
     */
    public function setParent(\Sincco\Excell\Worksheet $pSheet = null)
    {
        $this->workSheet = $pSheet;
    ...
    public function setRange($pRange = '')
    {
        ...
        if (strpos($pRange, ':') !== false) {
            ...
            throw new \Sincco\Excell\Exception('Autofilter must be set on a range of cells.');
        }
        ...
        if (empty($pRange)) {
            ...
            list($rangeStart, $rangeEnd) = \Sincco\Excell\Cell::rangeBoundaries($this->range);
            foreach ($this->columns as $key => $value) {
                $colIndex = \Sincco\Excell\Cell::columnIndexFromString($key);
    ...
    {
        if (empty($this->range)) {
            throw new \Sincco\Excell\Exception('No autofilter range is defined.');
        }

        $columnIndex = \Sincco\Excell\Cell::columnIndexFromString($column);
        list($rangeStart, $rangeEnd) = \Sincco\Excell\Cell::rangeBoundaries($this->range);
        if (($rangeStart[0] > $columnIndex) || ($rangeEnd[0] < $columnIndex)) {
            throw new \Sincco\Excell\Exception('Column is outside of current autofilter range.');
        }
    ...
    {
        list($rangeStart, $rangeEnd) = \Sincco\Excell\Cell::rangeBoundaries($this->range);
        $pColumn = \Sincco\Excell\Cell::stringFromColumnIndex($rangeStart[0] + $pColumnOffset - 1);
    ...
    public function setColumn($pColumn)
    {
        if ((is_string($pColumn)) && (!empty($pColumn))) {
            ...
            throw new \Sincco\Excell\Exception('Column is not within the autofilter range.');
        }
    ...
    private static function filterTestInDateGroupSet($cellValue, $dataSet)
    {
        ...
        if (is_numeric($cellValue)) {
            $dateValue = \Sincco\Excell\Shared\Date::excelToTimestamp($cellValue);
            if ($cellValue < 1) {
                //    Just the time part
    ...
    private static function filterTestInPeriodDateSet($cellValue, $monthSet)
    {
        ...
        if (is_numeric($cellValue)) {
            $dateValue = date('m', \Sincco\Excell\Shared\Date::excelToTimestamp($cellValue));
    ...
    {
        $rDateType = \Sincco\Excell\Calculation\Functions::getReturnDateType();
        \Sincco\Excell\Calculation\Functions::setReturnDateType(\Sincco\Excell\Calculation\Functions::RETURNDATE_PHP_NUMERIC);
        $val = $maxVal = null;

        $ruleValues = [];
        $baseDate = \Sincco\Excell\Calculation\DateTime::DATENOW();
        ...
        switch ($dynamicRuleType) {
            ...
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_TOMORROW:
                $maxVal = (int) \Sincco\Excell\Shared\Date::PHPtoExcel(strtotime('+1 day', $baseDate));
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel($baseDate);
                break;
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_YEARTODATE:
                $maxVal = (int) \Sincco\Excell\Shared\Date::PHPtoExcel(strtotime('+1 day', $baseDate));
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel(gmmktime(0, 0, 0, 1, 1, date('Y', $baseDate)));
                break;
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISYEAR:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTYEAR:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTYEAR:
                $maxVal = (int) \Sincco\Excell\Shared\Date::PHPToExcel(gmmktime(0, 0, 0, 31, 12, date('Y', $baseDate)));
                ++$maxVal;
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel(gmmktime(0, 0, 0, 1, 1, date('Y', $baseDate)));
                break;
            ...
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTQUARTER:
                ...
                $maxVal = (int) \Sincco\Excell\Shared\Date::PHPtoExcel(gmmktime(0, 0, 0, date('t', $baseDate), (1 + $thisQuarter) * 3, date('Y', $baseDate)));
                ++$maxVal;
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel(gmmktime(0, 0, 0, 1, 1 + $thisQuarter * 3, date('Y', $baseDate)));
                break;
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISMONTH:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTMONTH:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTMONTH:
                $maxVal = (int) \Sincco\Excell\Shared\Date::PHPtoExcel(gmmktime(0, 0, 0, date('t', $baseDate), date('m', $baseDate), date('Y', $baseDate)));
                ++$maxVal;
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel(gmmktime(0, 0, 0, 1, date('m', $baseDate), date('Y', $baseDate)));
                break;
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_THISWEEK:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_LASTWEEK:
            case AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTWEEK:
                $dayOfWeek = date('w', $baseDate);
                $val = (int) \Sincco\Excell\Shared\Date::PHPToExcel($baseDate) - $dayOfWeek;
                $maxVal = $val + 7;
                break;
        }
        ...
        \Sincco\Excell\Calculation\Functions::setReturnDateType($rDateType);
    ...
    private function calculateTopTenValue($columnID, $startRow, $endRow, $ruleType, $ruleValue)
    {
        ...
        $dataValues = \Sincco\Excell\Calculation\Functions::flattenArray($this->workSheet->rangeToArray($range, null, true, false));
                ...
                case AutoFilter\Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER:
                    ...
                    foreach ($rules as $rule) {
                        ...
                        if (($dynamicRuleType == AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE) ||
                            ...
                            $average = \Sincco\Excell\Calculation::getInstance()->calculateFormula($averageFormula, null, $this->workSheet->getCell('A1'));
            ...
            foreach ($columnFilterTests as $columnID => $columnFilterTest) {
                ...
                $result = $result &&
                    call_user_func_array(
                        ['\\Sincco\\Excell\\Worksheet\\AutoFilter', $columnFilterTest['method']],
    ...
    public function __clone()
    {
        ...
        foreach ($vars as $key => $value) {
            if (is_object($value)) {
                ...
                //    The columns array of \Sincco\Excell\Worksheet\AutoFilter objects
                $this->{$key} = [];
