...
namespace Sincco\Excell\Worksheet\AutoFilter\Column;
...
//    const AUTOFILTER_COLUMN_RULE_ALLDATESINQUARTER    = 'allDatesInQuarter';    //    <dynamicFilter type="Q2"/> for Quarter 2
     ...
     * Autofilter Column
     *
     * @var \Sincco\Excell\Worksheet\AutoFilter\Column
     */
    private $parent = null;
     ...
     * Create a new Rule
     *
     * @param \Sincco\Excell\Worksheet\AutoFilter\Column $pParent
     */
    public function __construct(\Sincco\Excell\Worksheet\AutoFilter\Column $pParent = null)
    {
        $this->parent = $pParent;
    }
    ...
    public function setRuleType($pRuleType = self::AUTOFILTER_RULETYPE_FILTER)
    {
        if (!in_array($pRuleType, self::$ruleTypes)) {
            throw new \Sincco\Excell\Exception('Invalid rule type for column AutoFilter Rule.');
        }
    ...
    public function setValue($pValue = '')
    {
        if (is_array($pValue)) {
            ...
            if (count($pValue) == 0) {
                throw new \Sincco\Excell\Exception('Invalid rule value for column AutoFilter Rule.');
            }
    ...
    public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL)
    {
        ...
        if ((!in_array($pOperator, self::$operators)) &&
            ...
            throw new \Sincco\Excell\Exception('Invalid operator for column AutoFilter Rule.');
        }
     ...
     *
     * @return \Sincco\Excell\Worksheet\AutoFilter\Column
     */
     ...
     *
     * @param  \Sincco\Excell\Worksheet\AutoFilter\Column
     * @return Rule
     */
    public function setParent(\Sincco\Excell\Worksheet\AutoFilter\Column $pParent = null)
    {
        $this->parent = $pParent;
