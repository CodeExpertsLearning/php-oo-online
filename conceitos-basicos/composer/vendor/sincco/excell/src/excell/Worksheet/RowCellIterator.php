...
class RowCellIterator extends CellIterator implements \Iterator
{
     ...
     *
     * @param  \Sincco\Excell\Worksheet   $subject        The worksheet to iterate over
     * @param  int               $rowIndex       The row that we want to iterate
     * @param  string                $startColumn    The column address at which to start iterating
     * @param  string                $endColumn      Optionally, the column address at which to stop iterating
     */
    public function __construct(\Sincco\Excell\Worksheet $subject = null, $rowIndex = 1, $startColumn = 'A', $endColumn = null)
    {
    ...
    {
        $startColumnIndex = \Sincco\Excell\Cell::columnIndexFromString($startColumn) - 1;
        $this->startColumn = $startColumnIndex;
        $this->adjustForExistingOnlyRange();
        $this->seek(\Sincco\Excell\Cell::stringFromColumnIndex($this->startColumn));
    ...
    public function resetEnd($endColumn = null)
    {
        ...
        $this->endColumn = \Sincco\Excell\Cell::columnIndexFromString($endColumn) - 1;
        $this->adjustForExistingOnlyRange();
    ...
    {
        $column = \Sincco\Excell\Cell::columnIndexFromString($column) - 1;
        if (($column < $this->startColumn) || ($column > $this->endColumn)) {
            throw new \Sincco\Excell\Exception("Column $column is out of range ({$this->startColumn} - {$this->endColumn})");
        } elseif ($this->onlyExistingCells && !($this->subject->cellExistsByColumnAndRow($column, $this->rowIndex))) {
            throw new \Sincco\Excell\Exception('In "IterateOnlyExistingCells" mode and Cell does not exist');
        }
    ...
    public function key()
    {
        return \Sincco\Excell\Cell::stringFromColumnIndex($this->position);
    }
    ...
    public function prev()
    {
        if ($this->position <= $this->startColumn) {
            throw new \Sincco\Excell\Exception(
                'Column is already at the beginning of range (' .
                \Sincco\Excell\Cell::stringFromColumnIndex($this->endColumn) . ' - ' .
                \Sincco\Excell\Cell::stringFromColumnIndex($this->endColumn) . ')'
            );
        }
    ...
    protected function adjustForExistingOnlyRange()
    {
        if ($this->onlyExistingCells) {
            ...
            if ($this->startColumn > $this->endColumn) {
                throw new \Sincco\Excell\Exception('No cells exist within the specified range');
            }
