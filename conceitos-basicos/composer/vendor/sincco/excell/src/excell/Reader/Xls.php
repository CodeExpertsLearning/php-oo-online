...
class Xls extends BaseReader implements IReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        try {
            ...
            $ole = new \Sincco\Excell\Shared\OLERead();
    ...
    /**
     * Reads names of the worksheets from a file, without parsing the whole file to a Excell object
     *
     * @param     string         $pFilename
     * @throws     Exception
     */
    ...
    public function load($pFilename)
    {
        ...
        // Initialisations
        $this->spreadsheet = new \Sincco\Excell\Spreadsheet();
        ...
        if (!$this->readDataOnly && $this->drawingGroupData) {
            $escherWorkbook = new \Sincco\Excell\Shared\Escher();
        ...
        foreach ($this->sheets as $sheet) {
            ...
            if (!$this->readDataOnly && $this->drawingData) {
                $escherWorksheet = new \Sincco\Excell\Shared\Escher();
            ...
            foreach ($this->objs as $n => $obj) {
                ...
                if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
                    ...
                    list($startColumn, $startRow) = \Sincco\Excell\Cell::coordinateFromString($spContainer->getStartCoordinates());
                    list($endColumn, $endRow) = \Sincco\Excell\Cell::coordinateFromString($spContainer->getEndCoordinates());
                    ...
                    $width = \Sincco\Excell\Shared\Xls::getDistanceX($this->phpSheet, $startColumn, $startOffsetX, $endColumn, $endOffsetX);
                    $height = \Sincco\Excell\Shared\Xls::getDistanceY($this->phpSheet, $startRow, $startOffsetY, $endRow, $endOffsetY);

                    // calculate offsetX and offsetY of the shape
                    $offsetX = $startOffsetX * \Sincco\Excell\Shared\Xls::sizeCol($this->phpSheet, $startColumn) / 1024;
                    $offsetY = $startOffsetY * \Sincco\Excell\Shared\Xls::sizeRow($this->phpSheet, $startRow) / 256;
                    ...
                    switch ($obj['otObjType']) {
                        ...
                        case 0x08:
                            ...
                            if ($blip = $BSE->getBlip()) {
                                ...
                                $drawing = new \Sincco\Excell\Worksheet\MemoryDrawing();
                                ...
                                switch ($blipType) {
                                    case \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_JPEG:
                                        $drawing->setRenderingFunction(\Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_JPEG);
                                        $drawing->setMimeType(\Sincco\Excell\Worksheet\MemoryDrawing::MIMETYPE_JPEG);
                                        break;
                                    case \Sincco\Excell\Shared\Escher\DggContainer\BstoreContainer\BSE::BLIPTYPE_PNG:
                                        $drawing->setRenderingFunction(\Sincco\Excell\Worksheet\MemoryDrawing::RENDERING_PNG);
                                        $drawing->setMimeType(\Sincco\Excell\Worksheet\MemoryDrawing::MIMETYPE_PNG);
                                        break;
                                }
            ...
            if ($this->version == self::XLS_BIFF8) {
                foreach ($this->sharedFormulaParts as $cell => $baseCell) {
                    list($column, $row) = \Sincco\Excell\Cell::coordinateFromString($cell);
                    if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($column, $row, $this->phpSheet->getTitle())) {
                        $formula = $this->getFormulaFromStructure($this->sharedFormulas[$baseCell], $cell);
                        $this->phpSheet->getCell($cell)->setValueExplicit('=' . $formula, \Sincco\Excell\Cell\DataType::TYPE_FORMULA);
                    }
                }
            }
            ...
            if ($definedName['isBuiltInName']) {
                            ...
                            if (count($explodes) == 2) {
                                if ($docSheet = $this->spreadsheet->getSheetByName($explodes[0])) {
                                    ...
                                    if (count($coordinateStrings) == 2) {
                                        list($firstColumn, $firstRow) = \Sincco\Excell\Cell::coordinateFromString($coordinateStrings[0]);
                                        list($lastColumn, $lastRow) = \Sincco\Excell\Cell::coordinateFromString($coordinateStrings[1]);
                ...
                if (count($explodes) == 2) {
                    if (($docSheet = $this->spreadsheet->getSheetByName($explodes[0])) ||
                        ...
                        $this->spreadsheet->addNamedRange(new \Sincco\Excell\NamedRange((string) $definedName['name'], $docSheet, $extractedRange, $localOnly, $scope));
                    }
                } else {
                    //    Named Value
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    ...
                    $value = \Sincco\Excell\Shared\StringHelper::convertEncoding($value, 'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = \Sincco\Excell\Shared\OLE::OLE2LocalDate(substr($this->summaryInformation, $secOffset + 4 + $offset, 8));
                    break;
            ...
            switch ($id) {
                case 0x01:    //    Code Page
                    $codePage = \Sincco\Excell\Shared\CodePage::numberToName($value);
                    break;
                case 0x02:    //    Title
    ...
    private function readDocumentSummaryInformation()
    {
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x40:    //    Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = \Sincco\Excell\Shared\OLE::OLE2LocalDate(substr($this->documentSummaryInformation, $secOffset + 4 + $offset, 8));
                    break;
                case 0x47:    //    Clipboard format
                    // not needed yet, fix later if necessary
                    break;
...
//                                                    ->setAuthor($author)
    ...
    private function readCodepage()
    {
        ...
        $this->codepage = \Sincco\Excell\Shared\CodePage::numberToName($codepage);
    }
    ...
    private function readDateMode()
    {
        ...
        \Sincco\Excell\Shared\Date::setExcelCalendar(\Sincco\Excell\Shared\Date::CALENDAR_WINDOWS_1900);
        if (ord($recordData{0}) == 1) {
            \Sincco\Excell\Shared\Date::setExcelCalendar(\Sincco\Excell\Shared\Date::CALENDAR_MAC_1904);
        }
    }
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            $objFont = new \Sincco\Excell\Style\Font();
            ...
            switch ($underlineType) {
                case 0x00:
                    break; // no underline
                case 0x01:
                    $objFont->setUnderline(\Sincco\Excell\Style\Font::UNDERLINE_SINGLE);
                    break;
                case 0x02:
                    $objFont->setUnderline(\Sincco\Excell\Style\Font::UNDERLINE_DOUBLE);
                    break;
                case 0x21:
                    $objFont->setUnderline(\Sincco\Excell\Style\Font::UNDERLINE_SINGLEACCOUNTING);
                    break;
                case 0x22:
                    $objFont->setUnderline(\Sincco\Excell\Style\Font::UNDERLINE_DOUBLEACCOUNTING);
                    break;
            }
    ...
    private function readXf()
    {
        ...
        $objStyle = new \Sincco\Excell\Style();
        ...
        if (!$this->readDataOnly) {
            ...
            } elseif (($code = \Sincco\Excell\Style\NumberFormat::builtInFormatCode($numberFormatIndex)) !== '') {
            ...
            $objStyle->getProtection()->setLocked($isLocked ? \Sincco\Excell\Style\Protection::PROTECTION_INHERIT : \Sincco\Excell\Style\Protection::PROTECTION_UNPROTECTED);

            // bit 1; mask 0x02; 1 = Formula is hidden
            $isHidden = (0x02 & $xfTypeProt) >> 1;
            $objStyle->getProtection()->setHidden($isHidden ? \Sincco\Excell\Style\Protection::PROTECTION_PROTECTED : \Sincco\Excell\Style\Protection::PROTECTION_UNPROTECTED);
            ...
            switch ($horAlign) {
                case 0:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_GENERAL);
                    break;
                case 1:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_LEFT);
                    break;
                case 2:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_CENTER);
                    break;
                case 3:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_RIGHT);
                    break;
                case 4:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_FILL);
                    break;
                case 5:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_JUSTIFY);
                    break;
                case 6:
                    $objStyle->getAlignment()->setHorizontal(\Sincco\Excell\Style\Alignment::HORIZONTAL_CENTER_CONTINUOUS);
                    break;
            }
            ...
            switch ($vertAlign) {
                case 0:
                    $objStyle->getAlignment()->setVertical(\Sincco\Excell\Style\Alignment::VERTICAL_TOP);
                    break;
                case 1:
                    $objStyle->getAlignment()->setVertical(\Sincco\Excell\Style\Alignment::VERTICAL_CENTER);
                    break;
                case 2:
                    $objStyle->getAlignment()->setVertical(\Sincco\Excell\Style\Alignment::VERTICAL_BOTTOM);
                    break;
                case 3:
                    $objStyle->getAlignment()->setVertical(\Sincco\Excell\Style\Alignment::VERTICAL_JUSTIFY);
                    break;
            }
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                if ($diagonalUp == false && $diagonalDown == false) {
                    $objStyle->getBorders()->setDiagonalDirection(\Sincco\Excell\Style\Borders::DIAGONAL_NONE);
                } elseif ($diagonalUp == true && $diagonalDown == false) {
                    $objStyle->getBorders()->setDiagonalDirection(\Sincco\Excell\Style\Borders::DIAGONAL_UP);
                } elseif ($diagonalUp == false && $diagonalDown == true) {
                    $objStyle->getBorders()->setDiagonalDirection(\Sincco\Excell\Style\Borders::DIAGONAL_DOWN);
                } elseif ($diagonalUp == true && $diagonalDown == true) {
                    $objStyle->getBorders()->setDiagonalDirection(\Sincco\Excell\Style\Borders::DIAGONAL_BOTH);
                }
    ...
    private function readStyle()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($isBuiltIn) {
                ...
                // user-defined; not supported by Excell
            }
        }
    }
    ...
    private function readSheet()
    {
        ...
        switch (ord($recordData{4})) {
            case 0x00:
                $sheetState = \Sincco\Excell\Worksheet::SHEETSTATE_VISIBLE;
                break;
            case 0x01:
                $sheetState = \Sincco\Excell\Worksheet::SHEETSTATE_HIDDEN;
                break;
            case 0x02:
                $sheetState = \Sincco\Excell\Worksheet::SHEETSTATE_VERYHIDDEN;
                break;
            default:
                $sheetState = \Sincco\Excell\Worksheet::SHEETSTATE_VISIBLE;
                break;
    ...
    private function readHorizontalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($cf, $r, \Sincco\Excell\Worksheet::BREAK_ROW);
            }
        }
    }
    ...
    private function readVerticalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                ...
                $this->phpSheet->setBreakByColumnAndRow($c, $rf, \Sincco\Excell\Worksheet::BREAK_COLUMN);
            }
        }
    }
    ...
    private function readPageSetup()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (!$isNotInit) {
                ...
                switch ($isPortrait) {
                    case 0:
                        $this->phpSheet->getPageSetup()->setOrientation(\Sincco\Excell\Worksheet\PageSetup::ORIENTATION_LANDSCAPE);
                        break;
                    case 1:
                        $this->phpSheet->getPageSetup()->setOrientation(\Sincco\Excell\Worksheet\PageSetup::ORIENTATION_PORTRAIT);
                        break;
                }
    ...
    private function readRk()
    {
        ...
        $columnString = \Sincco\Excell\Cell::stringFromColumnIndex($column);
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            // add cell
            $cell->setValueExplicit($numValue, \Sincco\Excell\Cell\DataType::TYPE_NUMERIC);
        }
    }
    ...
    private function readLabelSst()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if (($fmtRuns = $this->sst[$index]['fmtRuns']) && !$this->readDataOnly) {
                ...
                $richText = new \Sincco\Excell\RichText();
                $charPos = 0;
                $sstCount = count($this->sst[$index]['fmtRuns']);
                for ($i = 0; $i <= $sstCount; ++$i) {
                    if (isset($fmtRuns[$i])) {
                        $text = \Sincco\Excell\Shared\StringHelper::substring($this->sst[$index]['value'], $charPos, $fmtRuns[$i]['charPos'] - $charPos);
                        $charPos = $fmtRuns[$i]['charPos'];
                    } else {
                        $text = \Sincco\Excell\Shared\StringHelper::substring($this->sst[$index]['value'], $charPos, \Sincco\Excell\Shared\StringHelper::countCharacters($this->sst[$index]['value']));
                    }

                    if (\Sincco\Excell\Shared\StringHelper::countCharacters($text) > 0) {
                ...
                if ($this->readEmptyCells || trim($richText->getPlainText()) !== '') {
                    $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                    $cell->setValueExplicit($richText, \Sincco\Excell\Cell\DataType::TYPE_STRING);
                    $emptyCell = false;
                }
            } else {
                if ($this->readEmptyCells || trim($this->sst[$index]['value']) !== '') {
                    $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                    $cell->setValueExplicit($this->sst[$index]['value'], \Sincco\Excell\Cell\DataType::TYPE_STRING);
                    $emptyCell = false;
                }
            }
    ...
    private function readMulRk()
    {
        ...
        for ($i = 0; $i < $columns; ++$i) {
            $columnString = \Sincco\Excell\Cell::stringFromColumnIndex($colFirst + $i);
    ...
    private function readFormula()
    {
        ...
        if ($isPartOfSharedFormula) {
            ...
            $this->_baseCell = \Sincco\Excell\Cell::stringFromColumnIndex($baseCol) . ($baseRow + 1);
        }
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData{6}) == 0) && (ord($recordData{12}) == 255) && (ord($recordData{13}) == 255)) {
                ...
                $dataType = \Sincco\Excell\Cell\DataType::TYPE_STRING;
                ...
                $dataType = \Sincco\Excell\Cell\DataType::TYPE_BOOL;
                $value = (bool) ord($recordData{8});
            } elseif ((ord($recordData{6}) == 2)
                && (ord($recordData{12}) == 255)
                && (ord($recordData{13}) == 255)) {
                // Error formula. Error code is in +2
                $dataType = \Sincco\Excell\Cell\DataType::TYPE_ERROR;
                $value = Xls\ErrorCode::lookup(ord($recordData{8}));
            } elseif ((ord($recordData{6}) == 3)
                && (ord($recordData{12}) == 255)
                && (ord($recordData{13}) == 255)) {
                // Formula result is a null string
                $dataType = \Sincco\Excell\Cell\DataType::TYPE_NULL;
                $value = '';
            } else {
                // forumla result is a number, first 14 bytes like _NUMBER record
                $dataType = \Sincco\Excell\Cell\DataType::TYPE_NUMERIC;
            ...
            if (!$isPartOfSharedFormula) {
                ...
                try {
                    ...
                    $cell->setValueExplicit('=' . $formula, \Sincco\Excell\Cell\DataType::TYPE_FORMULA);
                } catch (\Sincco\Excell\Exception $e) {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            switch ($isError) {
                case 0: // boolean
                    ...
                    // add cell value
                    $cell->setValueExplicit($value, \Sincco\Excell\Cell\DataType::TYPE_BOOL);
                    break;
                case 1: // error type
                    $value = Xls\ErrorCode::lookup($boolErr);

                    // add cell value
                    $cell->setValueExplicit($value, \Sincco\Excell\Cell\DataType::TYPE_ERROR);
                    break;
            }
    ...
    private function readMulBlank()
    {
        ...
        if (!$this->readDataOnly && $this->readEmptyCells) {
            for ($i = 0; $i < $length / 2 - 3; ++$i) {
                $columnString = \Sincco\Excell\Cell::stringFromColumnIndex($fc + $i);
    ...
    private function readLabel()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ($this->readEmptyCells || trim($value) !== '') {
                $cell = $this->phpSheet->getCell($columnString . ($row + 1));
                $cell->setValueExplicit($value, \Sincco\Excell\Cell\DataType::TYPE_STRING);
    ...
    private function readBlank()
    {
        ...
        $columnString = \Sincco\Excell\Cell::stringFromColumnIndex($col);
    ...
    private function readWindow2()
    {
        ...
        if ($this->phpSheet->getSheetView()->getView() !== \Sincco\Excell\Worksheet\SheetView::SHEETVIEW_PAGE_LAYOUT) {
            ...
            $view = $isPageBreakPreview ? \Sincco\Excell\Worksheet\SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW : \Sincco\Excell\Worksheet\SheetView::SHEETVIEW_NORMAL;
    ...
    private function readPageLayoutView()
    {
        ...
        if ($fPageLayoutView === 1) {
            $this->phpSheet->getSheetView()->setView(\Sincco\Excell\Worksheet\SheetView::SHEETVIEW_PAGE_LAYOUT);
    ...
    private function readPane()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if ($this->frozen) {
                // frozen panes
                $this->phpSheet->freezePane(\Sincco\Excell\Cell::stringFromColumnIndex($px) . ($py + 1));
            } else {
                // unfrozen panes; split windows; not supported by Excell core
            }
        }
    }
    ...
    private function includeCellRangeFiltered($cellRangeAddress)
    {
        ...
        if ($this->getReadFilter() !== null) {
            ...
            $rangeBoundaries = \Sincco\Excell\Cell::getRangeBoundaries($cellRangeAddress);
            ++$rangeBoundaries[1][0];
    ...
    private function readHyperLink()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            foreach (\Sincco\Excell\Cell::extractAllCellReferencesInRange($cellRange) as $coordinate) {
    ...
    private function readDataValidation()
    {
        ...
        switch ($type) {
            case 0x00:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_NONE;
                break;
            case 0x01:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_WHOLE;
                break;
            case 0x02:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_DECIMAL;
                break;
            case 0x03:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_LIST;
                break;
            case 0x04:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_DATE;
                break;
            case 0x05:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_TIME;
                break;
            case 0x06:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_TEXTLENGTH;
                break;
            case 0x07:
                $type = \Sincco\Excell\Cell\DataValidation::TYPE_CUSTOM;
                break;
        }
        ...
        switch ($errorStyle) {
            case 0x00:
                $errorStyle = \Sincco\Excell\Cell\DataValidation::STYLE_STOP;
                break;
            case 0x01:
                $errorStyle = \Sincco\Excell\Cell\DataValidation::STYLE_WARNING;
                break;
            case 0x02:
                $errorStyle = \Sincco\Excell\Cell\DataValidation::STYLE_INFORMATION;
                break;
        }
        ...
        switch ($operator) {
            case 0x00:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_BETWEEN;
                break;
            case 0x01:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_NOTBETWEEN;
                break;
            case 0x02:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_EQUAL;
                break;
            case 0x03:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_NOTEQUAL;
                break;
            case 0x04:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_GREATERTHAN;
                break;
            case 0x05:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_LESSTHAN;
                break;
            case 0x06:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_GREATERTHANOREQUAL;
                break;
            case 0x07:
                $operator = \Sincco\Excell\Cell\DataValidation::OPERATOR_LESSTHANOREQUAL;
                break;
        }
        ...
        try {
            ...
            if ($type == \Sincco\Excell\Cell\DataValidation::TYPE_LIST) {
        ...
        foreach ($cellRangeAddresses as $cellRange) {
            ...
            foreach (\Sincco\Excell\Cell::extractAllCellReferencesInRange($stRange) as $coordinate) {
    ...
    private function readImData()
    {
        ...
        switch ($cf) {
            case 0x09: // Windows bitmap format
                ...
                $drawing = new \Sincco\Excell\Worksheet\Drawing();
    ...
    private function readBIFF8CellAddress($cellAddressStructure)
    {
        ...
        $column = \Sincco\Excell\Cell::stringFromColumnIndex(0x00FF & self::getInt2d($cellAddressStructure, 2));
    ...
    {
        list($baseCol, $baseRow) = \Sincco\Excell\Cell::coordinateFromString($baseCell);
        $baseCol = \Sincco\Excell\Cell::columnIndexFromString($baseCol) - 1;
        ...
        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(0x4000 & self::getInt2d($cellAddressStructure, 2))) {
            $column = \Sincco\Excell\Cell::stringFromColumnIndex($colIndex);
            $column = '$' . $column;
        } else {
            $colIndex = ($colIndex <= 127) ? $colIndex : $colIndex - 256;
            $column = \Sincco\Excell\Cell::stringFromColumnIndex($baseCol + $colIndex);
        }
    ...
    private function readBIFF5CellRangeAddressFixed($subData)
    {
        ...
        $fc = \Sincco\Excell\Cell::stringFromColumnIndex($fc);
        $lc = \Sincco\Excell\Cell::stringFromColumnIndex($lc);
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // bit: 7-0; mask 0x00FF; column index
        $fc = \Sincco\Excell\Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 4));
        ...
        // bit: 7-0; mask 0x00FF; column index
        $lc = \Sincco\Excell\Cell::stringFromColumnIndex(0x00FF & self::getInt2d($subData, 6));
    ...
    private function readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        if (!(0x4000 & self::getInt2d($subData, 4))) {
            ...
            $fc = \Sincco\Excell\Cell::stringFromColumnIndex($fcIndex);
            $fc = '$' . $fc;
        } else {
            // column offset
            $fcIndex = ($fcIndex <= 127) ? $fcIndex : $fcIndex - 256;
            $fc = \Sincco\Excell\Cell::stringFromColumnIndex($baseCol + $fcIndex);
        }
        ...
        $lc = \Sincco\Excell\Cell::stringFromColumnIndex($baseCol + $lcIndex);

        // bit: 14; mask 0x4000; (1 = relative column index, 0 = absolute column index)
        if (!(0x4000 & self::getInt2d($subData, 6))) {
            // absolute column index
            $lc = \Sincco\Excell\Cell::stringFromColumnIndex($lcIndex);
            $lc = '$' . $lc;
        } else {
            // column offset
            $lcIndex = ($lcIndex <= 127) ? $lcIndex : $lcIndex - 256;
    ...
    private static function encodeUTF16($string, $compressed = false)
    {
        ...
        return \Sincco\Excell\Shared\StringHelper::convertEncoding($string, 'UTF-8', 'UTF-16LE');
    }
    ...
    {
        return \Sincco\Excell\Shared\StringHelper::convertEncoding($string, 'UTF-8', $this->codepage);
    }
