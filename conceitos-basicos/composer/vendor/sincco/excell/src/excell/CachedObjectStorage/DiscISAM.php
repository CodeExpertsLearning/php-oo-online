...
class DiscISAM extends CacheBase implements ICache
{
    ...
    public function copyCellCollection(\Sincco\Excell\Worksheet $parent)
    {
        ...
        $newFileName = $this->cacheDirectory . '/Excell.' . $baseUnique . '.cache';
    ...
    public function __construct(\Sincco\Excell\Worksheet $parent, $arguments)
    {
        $this->cacheDirectory = ((isset($arguments['dir'])) && ($arguments['dir'] !== null))
                                    ? $arguments['dir']
                                    : \Sincco\Excell\Shared\File::sysGetTempDir();

        parent::__construct($parent);
        if (is_null($this->fileHandle)) {
            $baseUnique = $this->getUniqueID();
            $this->fileName = $this->cacheDirectory . '/Excell.' . $baseUnique . '.cache';
            $this->fileHandle = fopen($this->fileName, 'a+');
        }
    }
