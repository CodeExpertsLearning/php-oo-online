...
class Memcache extends CacheBase implements ICache
{
        ...
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!$this->memcache->replace($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                if (!$this->memcache->add($this->cachePrefix . $this->currentObjectID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new \Sincco\Excell\Exception("Failed to store cell {$this->currentObjectID} in MemCache");
                }
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new \Sincco\Excell\Exception('Cell entry ' . $pCoord . ' no longer exists in MemCache');
            }
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new \Sincco\Excell\Exception("Cell entry {$cellID} no longer exists in MemCache");
                }
                if (!$this->memcache->add($newCachePrefix . $cellID . '.cache', $obj, null, $this->cacheTime)) {
                    $this->__destruct();
                    throw new \Sincco\Excell\Exception("Failed to store cell {$cellID} in MemCache");
                }
            }
        }
     ...
     *
     * @param   \Sincco\Excell\Worksheet    $parent        The worksheet for this cell collection
     * @param   mixed[]        $arguments    Additional initialisation arguments
     * @throws  \Sincco\Excell\Exception
     */
    public function __construct(\Sincco\Excell\Worksheet $parent, $arguments)
    {
        ...
        if (is_null($this->cachePrefix)) {
            ...
            if (!$this->memcache->addServer($memcacheServer, $memcachePort, false, 50, 5, 5, true, [$this, 'failureCallback'])) {
                throw new \Sincco\Excell\Exception("Could not connect to MemCache server at {$memcacheServer}:{$memcachePort}");
            }
    ...
    {
        throw new \Sincco\Excell\Exception("memcache {$host}:{$port} failed");
    }
