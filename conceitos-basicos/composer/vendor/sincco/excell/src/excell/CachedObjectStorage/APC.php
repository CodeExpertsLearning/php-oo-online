    ...
    protected function storeData()
    {
        if ($this->currentCellIsDirty && !empty($this->currentObjectID)) {
            ...
            if (!apc_store(
                ...
                $this->__destruct();
                throw new \Sincco\Excell\Exception('Failed to store cell ' . $this->currentObjectID . ' in APC');
            }
    ...
    public function isDataSet($pCoord)
    {
        ...
        if (parent::isDataSet($pCoord)) {
            ...
            if ($success === false) {
                ...
                throw new \Sincco\Excell\Exception('Cell entry ' . $pCoord . ' no longer exists in APC cache');
            }
        ...
        foreach ($cacheList as $cellID) {
            if ($cellID != $this->currentObjectID) {
                ...
                if ($obj === false) {
                    ...
                    throw new \Sincco\Excell\Exception('Cell entry ' . $cellID . ' no longer exists in APC');
                }
                if (!apc_store($newCachePrefix . $cellID . '.cache', $obj, $this->cacheTime)) {
                    $this->__destruct();
                    throw new \Sincco\Excell\Exception('Failed to store cell ' . $cellID . ' in APC');
                }
            }
        }
